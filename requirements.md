...
FUN-INPUT-CRASH - Rather than displaying the available commands and their respective functionalities
when an invalid command is inputted, the program will crash.

FUN-INVALID-MISTOFFELEES-RENAME - If the user, after renting the cat Mistoffelees, tries to rename
the cat, the program shall inform the user that this cat's ID is invalid.

FUN-RENT-INVALID-CAT - If the user attempts to rent an invalid cat ID, the system shall
allow the user to rent the cat.